...
def analyze_tweet_sentiment(tweet):
    ...
    sentiments = [sentiment_value(get_word_sentiment(word)) for word in tweetText if has_sentiment(get_word_sentiment(word))]
    if len(sentiments) != 0:
...
def find_centroid(polygon):
    ...
    for position in range(len(polygon)):
        if position <= (len(polygon) - 2):
            polygonArea += (latitude(polygon[position])*longitude(polygon[position+1]) - latitude(polygon[position+1])*longitude(polygon[position]))
    polygonArea = polygonArea * (1/2)

    if polygonArea == 0:
        xCoord = latitude(polygon[0])
        yCoord = longitude(polygon[0])
    else:
        for position in range(len(polygon)):
            if position <= (len(polygon) - 2):
                xCoord += (latitude(polygon[position]) + latitude(polygon[position+1])) * (latitude(polygon[position])*longitude(polygon[position+1]) - latitude(polygon[position+1])*longitude(polygon[position]))
        xCoord = (1/(6 * polygonArea)) * xCoord
        for position in range(len(polygon)):
            if position <= (len(polygon) - 2):
                yCoord += (longitude(polygon[position]) + longitude(polygon[position+1])) * (latitude(polygon[position])*longitude(polygon[position+1]) - latitude(polygon[position+1])*longitude(polygon[position]))
        yCoord = (1/(6 * polygonArea)) * yCoord

    return xCoord, yCoord, abs(polygonArea)
...
def find_state_center(polygons):
    ...
    for i in range(len(polygons)):
        y += list(find_centroid(polygons[i]))[1] * list(find_centroid(polygons[i]))[2]
        x += list(find_centroid(polygons[i]))[0] * list(find_centroid(polygons[i]))[2]
        xy += list(find_centroid(polygons[i]))[2]
    x = x/xy
    y = y/xy
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        for state in us_states.keys():
            ...
            if distance < 3closestDistance:
                closestDistance = distance
                closestState = state
        if closestState not in tweets_by_state.keys():
            tweets_by_state[closestState] = [tweet]
        else:
            tweets_by_state[closestState].append(tweet)
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state.keys():
        if state_average(tweets_by_state[state]) != 0:
            averaged_state_sentiments[state] = state_average(tweets_by_state[state])
